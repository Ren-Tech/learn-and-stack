...
const Primary = () => {
  ...
  // Function to determine instant assess button position - UPDATED to move left by 1 inch (96px)
  ...
  // Get right position for instant assess button - UPDATED
  ...
  return (
    <div className={`min-h-screen bg-white relative overflow-x-hidden ${showPortraitLock ? 'backdrop-blur-sm' : ''}`} onClick={handleContainerClick}>
      ...
      {useBbcLayout ? (
        ...
        <div className={`w-full h-screen bg-gradient-to-br from-green-50 via-blue-50 to-indigo-50 overflow-y-auto ${showPortraitLock ? 'blur-sm' : ''}`}>
          ...
          <div className="sticky top-0 left-0 right-0 h-8 bg-gradient-to-r from-green-600 to-blue-600 z-40 flex items-center px-4">
            <div className="flex items-center space-x-2">
              <div className="w-6 h-6 bg-white rounded-full flex items-center justify-center">
                <div className="w-3 h-3 bg-green-500 rounded-full"></div>
              </div>
              <span className="text-white font-bold text-sm tracking-tight">PRIMARY EDUCATION</span>
            </div>
      ...
      <div className="fixed inset-0 z-40 pointer-events-none">
        {/* Instant Assessment Button - UPDATED position and size */}
        {showInstantAssess && !isMenuOpen && (
          ...
          >
            <div className="relative" style={{ 
              width: getResponsiveSize('150px', '180px', '220px', '100px', '90px'), // Increased sizes
              height: getResponsiveSize('150px', '180px', '220px', '100px', '90px') // Increased sizes
            }}>
              <div className="absolute inset-0">
                {assessmentCategories.map((category, index) => {
                  const radius = getResponsiveSize(100, 120, 150, 60, 55); // Increased radius
                  const position = getCirclePosition(category.angle, radius, index);
                  const circleSize = getResponsiveSize('60px', '70px', '85px', '42px', '38px'); // Increased circle sizes
              ...
              <button 
                ...
                style={{ 
                  width: getResponsiveSize('75px', '90px', '105px', '50px', '45px'), // Increased main button sizes
                  height: getResponsiveSize('75px', '90px', '105px', '50px', '45px'), // Increased main button sizes
