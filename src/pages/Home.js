...
const Home = () => {
  ...
  const isVerySmallLandscape = isMobileLandscape && windowSize.width < 500;
  const isTabletPortrait = isTablet && !isLandscape;

  const getResponsiveSize = (mobile, tablet, desktop, landscapeMobile = null, smallLandscape = null, verySmallLandscape = null) => {
    if (isVerySmallLandscape && verySmallLandscape !== null) return verySmallLandscape;
  ...
  // Enhanced circle positioning for all screen sizes
  const getCirclePosition = (angle, radius, index) => {
    if (isMobileLandscape) {
      // For mobile landscape, arrange in a compact horizontal line
      const totalCircles = assessmentCategories.length;
      const spacing = isVerySmallLandscape ? 70 : isSmallMobileLandscape ? 85 : 100;
    ...
    if (isTablet) {
      // For tablet, arrange circles in a horizontal line
      const totalCircles = assessmentCategories.length;
      const spacing = 180;
      const startX = -((totalCircles - 1) * spacing) / 2;
      return { 
  ...
  const getHomeImage = () => {
    if (isVerySmallLandscape) return '/images/home_landscape.png';
  ...
  const getButtonPosition = () => {
    if (isVerySmallLandscape) return 'top-[45%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 scale-75';
    if (isSmallMobileLandscape) return 'top-[45%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 scale-85';
  ...
  // Enhanced ninja positioning
  const getNinjaPosition = () => {
    if (isVerySmallLandscape) return 'mb-[-5px] scale-50';
    if (isSmallMobileLandscape) return 'mb-[-8px] scale-60';
    if (isMobileLandscape) return 'mb-[-5px] scale-70';
    if (isMobile) return isLandscape ? 'mb-[-5px] scale-90' : 'mb-[-15px]';
    if (isTablet) return 'mb-[20px]';
    return 'mb-[-40px]';
  };

  // Enhanced ninja dialog positioning
  const getNinjaDialogPosition = () => {
    if (isVerySmallLandscape) return 'top-0 -right-1 translate-x-full scale-65';
    if (isSmallMobileLandscape) return 'top-1 -right-2 translate-x-full scale-70';
    if (isMobileLandscape) return 'top-2 -right-1 translate-x-full scale-85';
  ...
  // Enhanced plus menu positioning
  const getPlusMenuPosition = () => {
    if (isVerySmallLandscape) return 'bottom-1 left-1 scale-60';
    if (isSmallMobileLandscape) return 'bottom-1 left-1 scale-70';
    if (isMobileLandscape) return 'bottom-2 left-2 scale-80';
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-10">
        ...
        {!isMenuOpen && (
          ...
          >
            <div className="relative" style={{ 
              width: getResponsiveSize('200px', '250px', '280px', '180px', '160px', '140px'), 
              height: getResponsiveSize('200px', '250px', '280px', '180px', '160px', '140px') 
            }}>
              <div className="absolute inset-0">
                {assessmentCategories.map((category, index) => {
                  const radius = getResponsiveSize(120, 150, 180, 100, 90, 80);
                  const position = getCirclePosition(category.angle, radius, index);
                  const circleSize = getResponsiveSize('80px', '100px', '112px', '70px', '60px', '50px');
                  ...
                  return (
                    <div key={category.title} className={`absolute transition-all duration-700 ease-out ${shouldShowMiniCircles ? 'opacity-100 scale-100' : 'opacity-0 scale-0'}`}
                      ...
                      {/* Connecting lines - only show for non-mobile */}
                      {!isMobile && (
                        <svg className={`absolute transition-all duration-500 ${shouldShowMiniCircles ? 'opacity-100' : 'opacity-0'}`}
                          style={{ 
                            ...
                            width: isTablet || isMobileLandscape ? `${Math.abs(position.x) * 2 + 50}px` : `${radius + 50}px`, 
                            height: isTablet || isMobileLandscape ? '100px' : `${radius + 50}px`, 
                            pointerEvents: 'none', 
                          ...
                          <circle 
                            cx={isTablet || isMobileLandscape ? `${(Math.abs(position.x) * 2 + 50) / 2 + position.x}` : `${(radius + 50) / 2 - position.x}`} 
                            cy={isTablet || isMobileLandscape ? `${50 + position.y}` : `${(radius + 50) / 2 - position.y}`} 
                            r="4" 
                            fill="#fbbf24" 
                      ...
                      >
                        ...
                        <div className="relative z-10 flex flex-col items-center justify-center h-full p-2">
                          <span className="text-white font-black tracking-wider mb-1 drop-shadow-lg text-center" style={{ 
                            textShadow: '2px 2px 4px rgba(0,0,0,0.5)', 
                            fontSize: getResponsiveSize('0.75rem', '0.875rem', '1.125rem', '0.7rem', '0.65rem', '0.6rem') 
                          }}>
                            {category.title}
                          </span>
                        </div>
              ...
              <button 
                ...
                style={{ 
                  width: getResponsiveSize('100px', '120px', '160px', '90px', '80px', '70px'), 
                  height: getResponsiveSize('100px', '120px', '160px', '90px', '80px', '70px'), 
              ...
              >
                ...
                <div className={`absolute rounded-full transition-opacity duration-700 ${!isMobile && is3DButtonHovered ? 'opacity-100' : 'opacity-60'}`} style={{ inset: getResponsiveSize('1.5rem', '1.75rem', '1.5rem', '1rem', '0.875rem', '0.75rem'), background: 'radial-gradient(circle at 30% 30%, rgba(255,255,255,0.4) 0%, transparent 60%)', animation: !isMobile && is3DButtonHovered ? 'pulse 1.5s ease-in-out infinite' : 'none' }}></div>
                <div className="relative z-10 flex flex-col items-center justify-center h-full">
                  <div className="relative mb-1">
                    <div className="absolute inset-0 bg-white/20 rounded-full blur-md"></div>
                    <svg className="text-white drop-shadow-lg relative z-10" style={{ 
                      width: getResponsiveSize('2rem', '2.5rem', '3.5rem', '1.75rem', '1.5rem', '1.25rem'), 
                      height: getResponsiveSize('2rem', '2.5rem', '3.5rem', '1.75rem', '1.5rem', '1.25rem') 
                  ...
                  <div className="text-center">
                    <span className="text-white font-black tracking-wider drop-shadow-lg block" style={{ 
                      ...
                      fontSize: getResponsiveSize('0.75rem', '0.875rem', '1.25rem', '0.7rem', '0.65rem', '0.6rem') 
                    }}>
                      INSTANT
                    </span>
                    <div className="text-white font-bold tracking-wide drop-shadow-lg" style={{ 
                      textShadow: '1px 1px 2px rgba(0,0,0,0.5)', 
                      fontSize: getResponsiveSize('0.6rem', '0.7rem', '0.875rem', '0.55rem', '0.5rem', '0.45rem') 
                    }}>
                      ASSESS
                    </div>
                    <span className="text-white/90 font-semibold mt-0.5 block" style={{ 
                      textShadow: '1px 1px 2px rgba(0,0,0,0.5)', 
                      fontSize: getResponsiveSize('0.5rem', '0.6rem', '0.5rem', '0.45rem', '0.4rem', '0.35rem') 
                    }}>
                      Powered by KAI
                    </span>
                  </div>
        ...
        <nav className={`btn-pluss-wrapper fixed z-40 flex flex-col items-center transition-all duration-300 ${getPlusMenuPosition()}`}
          ...
          <div className="flex flex-col items-center mb-2">
            <div className="bg-white rounded-2xl px-3 py-2 shadow-lg border border-gray-200 mb-2 relative" style={{ 
              minWidth: getResponsiveSize('100px', '120px', '120px', '90px', '80px', '70px'), 
              minHeight: getResponsiveSize('35px', '40px', '40px', '30px', '25px', '20px') 
            }}>
              <div className="font-medium text-gray-800 text-center" style={{ fontSize: getResponsiveSize('0.75rem', '0.875rem', '0.875rem', '0.7rem', '0.65rem', '0.6rem') }}>
                {displayedText}
            ...
            <img 
              ...
              style={{ 
                width: getResponsiveSize('2.5rem', '3rem', '3rem', '2rem', '1.75rem', '1.5rem'), 
                height: getResponsiveSize('2.5rem', '3rem', '3rem', '2rem', '1.75rem', '1.5rem') 
              }} 
            />
          </div>
          ...
          <div className={`btn-pluss bg-blue-900 overflow-hidden flex flex-col items-center transition-all duration-600 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
            ...
            width: isHovered ? getResponsiveSize('10rem', '11rem', '11rem', '9rem', '8rem', '7rem') : getResponsiveSize('3rem', '3rem', '3rem', '2.5rem', '2.25rem', '2rem')
          }}>
            <div className={`bg-red-600 rounded-full flex items-center justify-center text-white font-bold transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
              ...
              width: getResponsiveSize('2.5rem', '3rem', '3rem', '2rem', '1.75rem', '1.5rem'), 
              height: getResponsiveSize('2.5rem', '3rem', '3rem', '2rem', '1.75rem', '1.5rem'), 
              fontSize: getResponsiveSize('1rem', '1.25rem', '1.25rem', '0.875rem', '0.8rem', '0.7rem') 
            }}>+</div>
            ...
            <ul className={`transition-all duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] px-3 w-full ${
              ...
              {menuItems.map((item, index) => (
                <li key={index} className={`bg-white rounded-lg transition-all duration-600 ease-[cubic-bezier(0.34,1.56,0.64,1)] transform border border-gray-100 shadow-sm ${
                  ...
                  style={{ 
                    height: isHovered ? getResponsiveSize('2.5rem', '3rem', '3rem', '2.25rem', '2rem', '1.75rem') : '0', 
                  ...
                  <div className="text-blue-900 font-medium block w-full h-full flex flex-col items-center justify-center transition-colors duration-300 hover:text-blue-700 p-1" style={{ 
                    fontSize: getResponsiveSize('0.7rem', '0.8rem', '0.875rem', '0.65rem', '0.6rem', '0.55rem') 
                  }}>
                    <span className="font-semibold">{item.text}</span>
                    <span className="text-gray-600 text-center" style={{ 
                      fontSize: getResponsiveSize('0.6rem', '0.7rem', '0.75rem', '0.55rem', '0.5rem', '0.45rem') 
          ...
          <div className="relative">
            ...
            <div className={`absolute transform ${getNinjaDialogPosition()}`}>
              <div className="bg-white rounded-2xl px-3 py-2 shadow-lg border border-gray-200 relative" style={{ 
                maxWidth: getResponsiveSize('140px', '160px', '180px', '120px', '100px', '90px'), 
                minHeight: getResponsiveSize('60px', '70px', '80px', '50px', '45px', '40px') 
              }}>
                ...
                <div className="absolute transform -translate-y-1/2 rotate-45 bg-white border-l border-b border-gray-200" style={{ 
                  bottom: getResponsiveSize('1.5rem', '1.75rem', '2rem', '1.25rem', '1rem', '0.875rem'), 
                  left: '-0.25rem', 
                  width: getResponsiveSize('0.5rem', '0.6rem', '0.75rem', '0.4rem', '0.35rem', '0.3rem'), 
                  height: getResponsiveSize('0.5rem', '0.6rem', '0.75rem', '0.4rem', '0.35rem', '0.3rem') 
                }}></div>
              </div>
            </div>
          </div>
