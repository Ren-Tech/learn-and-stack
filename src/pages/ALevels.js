...
const ALevels = () => {
  ...
  const assessmentCategories = [
    ...
    { title: "Humanities", gradient: "from-green-600 via-green-500 to-emerald-500", shadow: "shadow-green-500/50", glow: "shadow-green-400", angle: 20, href: "/humanities-assessment" },
    { title: "Languages", gradient: "from-orange-600 via-orange-500 to-yellow-500", shadow: "shadow-orange-500/50", glow: "shadow-orange-400", angle: 60, href: "/languages-assessment" }
  ];
  ...
  // Circle position calculation for mobile only - UPDATED with larger spacing to match ElevenPlus
  const getCirclePosition = (angle, radius, index) => {
    ...
    if (isMobilePortrait && showMiniCircles) {
      ...
      const spacingX = 90; // Increased horizontal spacing to match ElevenPlus
      const spacingY = 90; // Increased vertical spacing to match ElevenPlus
      return { 
    ...
    if (isMobileLandscape && showMiniCircles) {
      ...
      const spacing = 80; // Increased spacing to match ElevenPlus
      const startX = -((totalCircles - 1) * spacing) / 2;
      return { 
        x: startX + (index * spacing), 
        y: -90 // Moved up slightly to match ElevenPlus
      };
    }
    
    if (isTablet) {
      const totalCircles = assessmentCategories.length;
      const spacing = 200; // Increased spacing to match ElevenPlus
      const startX = -((totalCircles - 1) * spacing) / 2;
      return { 
        x: startX + (index * spacing), 
        y: -90
  ...
  // Function to determine ninja position - UPDATED with -4rem margin for PC (SAME AS ELEVENPLUS AND GCSES)
  ...
  // Function to determine instant assess button position - UPDATED to move left by 1 inch (96px) to match ElevenPlus
  const getInstantAssessButtonPosition = () => {
    if (isDesktopLandscape) return 'top-1/2 transform -translate-y-1/2 scale-100';
    if (isMobilePortrait) return 'bottom-4 transform scale-85'; // Increased from 0.75 to match ElevenPlus
    if (isMobileLandscape) return 'bottom-100 transform scale-75'; // Increased from 0.65 to match ElevenPlus
    if (isTablet) return 'bottom-6 transform scale-100'; // Increased from 0.90 to match ElevenPlus
    return 'bottom-8 transform scale-100';
  };

  // Get right position for instant assess button - UPDATED to match ElevenPlus
  const getInstantAssessRightPosition = () => {
    if (isDesktopLandscape) return '128px'; // Desktop: moved left by 96px (1 inch) to match ElevenPlus
    if (isMobilePortrait) return '20px'; // Mobile portrait: moved left to match ElevenPlus
    if (isMobileLandscape) return '60px'; // Mobile landscape: moved left to match ElevenPlus
    if (isTablet) return '30px'; // Tablet: moved left to match ElevenPlus
    return '40px'; // Default: moved left to match ElevenPlus
  };
  ...
  // Ninja dialog typing animation - REMOVED since we're removing the dialog (SAME AS ELEVENPLUS AND GCSES)
  useEffect(() => {
  ...
  return (
    <div className={`min-h-screen bg-white relative overflow-x-hidden ${showPortraitLock ? 'backdrop-blur-sm' : ''}`} onClick={handleContainerClick}>
      ...
      {useBbcLayout ? (
        ...
        <div className={`w-full h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 overflow-y-auto ${showPortraitLock ? 'blur-sm' : ''}`}>
          ...
          {/* A-Level Info for Mobile BBC Layout - SIMPLE FORMAT */}
          <div className="px-4 pt-6">
            {/* Title */}
            <h1 className="text-2xl font-bold text-gray-900 mb-4">A-Level Preparation</h1>
            
            {/* Simple text lines - Only 1 line */}
            <div className="mb-4">
              <p className="text-lg text-gray-700">A-Level Exam Apps</p>
            </div>
            
            {/* KS5 Image and Text side by side - Reduced image size by 20% */}
            <div className="flex items-start space-x-4 mb-4">
              ...
              <div className="flex-1">
                <p className="text-base text-gray-700 leading-relaxed line-clamp-3"> {/* Added line-clamp-3 */}
                  Advanced Levels (A-Levels) are subject-based academic qualification typically
                  studied over 2 years (16-18 years old after completing GCSEs). Taught in classrooms 
                  with some via online lessons and assessed by exams. Note that some subjects can
                  Include certain amount of coursework.
                </p>
              </div>
            </div>
          </div>
        ...
        // Original layout for other screen sizes with A-Level info
        <div className={`relative ${showPortraitLock ? 'blur-sm' : ''}`}>
          ...
          {/* A-Level Information Section - SIMPLE FORMAT */}
          <div className={`absolute top-4 left-4 z-30 p-4 max-w-lg ${isDesktop ? 'pc-text-size' : ''}`}>
            {/* Title */}
            <h1 className={`${isDesktop ? 'text-2xl' : 'text-3xl'} font-bold text-gray-900 mb-4`}>A-Level Preparation</h1>
            
            {/* Simple text lines - Only 1 line */}
            <div className="mb-4">
              <p className={`${isDesktop ? 'text-lg' : 'text-xl'} text-gray-700`}>A-Level Exam Apps</p>
            </div>
      ...
      <div className="fixed inset-0 z-40 pointer-events-none">
        {/* Instant Assessment Button - UPDATED position and size to match ElevenPlus */}
        {showInstantAssess && !isMenuOpen && (
          ...
          >
            <div className="relative" style={{ 
              width: getResponsiveSize('150px', '180px', '220px', '100px', '90px'), // Increased sizes to match ElevenPlus
              height: getResponsiveSize('150px', '180px', '220px', '100px', '90px') // Increased sizes to match ElevenPlus
            }}>
              <div className="absolute inset-0">
                {assessmentCategories.map((category, index) => {
                  const radius = getResponsiveSize(100, 120, 150, 60, 55); // Increased radius to match ElevenPlus
                  const position = getCirclePosition(category.angle, radius, index);
                  const circleSize = getResponsiveSize('60px', '70px', '85px', '42px', '38px'); // Increased circle sizes to match ElevenPlus
              ...
              <button 
                ...
                style={{ 
                  width: getResponsiveSize('75px', '90px', '105px', '50px', '45px'), // Increased main button sizes to match ElevenPlus
                  height: getResponsiveSize('75px', '90px', '105px', '50px', '45px'), // Increased main button sizes to match ElevenPlus
        ...
        {/* Floating Plus Menu with Robot - Responsive Positioning (SAME AS ELEVENPLUS AND GCSES) */}
        <nav 
        ...
        >
          {/* Robot Image without Dialog (SAME AS ELEVENPLUS AND GCSES) */}
        ...
        {/* Ninja Image without Dialog - Responsive Positioning with 20rem size for PC and -4rem margin (SAME AS ELEVENPLUS AND GCSES) */}
            ...
            <img 
              ...
              style={{ 
                width: getResponsiveSize(
                  ...
                  '14rem', 
                  '20rem', // Set to 20rem for PC (SAME AS ELEVENPLUS AND GCSES)
                  '6rem', 
                  '5rem'
                ), 
      ...
      <style jsx>{`
        ...
        @media (min-width: 1024px) {
          ...
          /* Specific adjustment for A-Level text */
          .pc-text-size .text-lg {
            font-size: 0.875rem !important; /* Reduced from lg to base */
          }
        }

        /* Optional: Add hover effect for PC (SAME AS ELEVENPLUS AND GCSES) */
        @media (min-width: 1024px) {
          .pc-ninja:hover {
